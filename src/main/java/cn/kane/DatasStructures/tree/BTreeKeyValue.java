 ...
 *            - 值类型
 */
public class BTreeKeyValue<K, V> {
	...
	/**
	 * 为了简单起见，暂时只支持整型的key， 等到工作完成后，支持泛型。
	 * <p/>
	 * 
	 * TODO 需要考虑并发情况下的存取。
	 */
	private static class BTreeNode {
		...
		/**
		 * 返回关键字的个数。如果是非叶子节点，该节点的 子节点个数为({@link #size()} + 1)。
		 * 
		 * @return 关键字的个数
		 */
		public int size() {
		...
		/**
		 * 在节点中查找给定的<code>key</code>，如果节点中存在给定的 <code>key</code>，则返回一个
		 * <code>SearchResult</code>， 标识此次查找成功，给定<code>key</code>在节点中的索引和给定
		 * <code>key</code>对应的值。如果不存在，则返回<code>SearchResult</code> 标识此次查找失败，给定
		 * <code>key</code>应该插入的位置，该<code>key</code> 对应的值为null。
		 * <p/>
		...
		/**
		 * 将给定的<code>key</code>追加到节点的末尾， 一定要确保调用该方法之后，节点中的关键字还是 以非降序存放。
		 * 
		 * @param key
		 *            - 给定的键值
		 */
		...
		/**
		 * 在该节点中插入给定的<code>key</code>， 该方法保证插入之后，其键值还是以非降序存放。
		 * <p/>
		 * 不过该方法的时间复杂度为O(t)。
		 * <p/>
		 * TODO 需要考虑键值是否可以重复。
		...
		/**
		 * 在该节点中给定索引的位置插入给定的<code>key</code>， 你需要自己保证<code>key</code>插入了正确的位置。
		 * 
		 * @param key
		 *            - 给定的键值
		 * @param index
		...
		public void insertKey(Integer key, int index) {
			/*
			 * TODO 通过新建一个ArrayList来实现插入真的很恶心，先这样吧 要是有类似C中的reallocate就好了。
			 */
	...
	public BTreeKeyValue() {
		root = new BTreeNode();
		root.setLeaf(true);
	}

	public BTreeKeyValue(int t) {
		this();
		this.t = t;
		minKeySize = t - 1;
		maxKeySize = 2 * t - 1;
	 ...
	 * 搜索给定的<code>key</code>。
	 * <p/>
	 * TODO 需要重新定义返回结果，应该返回 <code>key</code>对应的值。
	 * 
	 * @param key
	 *            - 给定的键值
	 * @return TODO 得返回值类型
	...
	public static void main(String[] args) {
		...
		BTreeKeyValue<Integer, Byte[]> btree = new BTreeKeyValue<Integer, Byte[]>();
		for (int i = 0; i < 10; ++i) {
