...
/**
 * 快速排序算法：任取array中一数据data（一般为header数据），将array分为两部分，一部分比data小，一部分比其大；递归调用直至startIndex>=endIndex（使其有序）
 * 
 * @author jzj
 * @date 2009-12-9
 * 
 * @param <E>
 */
public class QuickSort<E extends Comparable<E>> extends Sortors<E> {
	/**
	...
	private int partition3(E[] array, int low, int high, Comparator<E> c) {
		...
		while (true) {
			// 如果高指针未超出低指针
			while (low < high) {
				// 如果低指针指向的元素大于或等于中枢元素时表示找到了，退出，注：等于时也要
				if (c.compare(array[low], array[pivot]) >= 0) {
					break;
